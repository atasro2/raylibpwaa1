...
parser.add_argument("-o", "--output", dest="outfile", required=True, help="Output animation file name",)
parser.add_argument("-i", "--input", dest="infile", required=True, help="input animation definition yaml",)
parser.add_argument("-4", "--four", action='store_true', default=False, help="Support 2-bit palette mode",)

args = parser.parse_args()

with open(args.infile) as f:
    animdef = yaml.safe_load(f)

pixdata = {}
inpath = Path(args.infile)
with open(args.outfile + ".pix", "wb") as f:
    sheetoffset = 0
    for sheet in animdef["sheets"]:
        size = 0
        sheet_name = os.path.splitext(os.path.basename(sheet))[0]
        sheet = inpath.parent / sheet
        #p =  Path(sheet)
        sheetyml = read_sheet_yaml_file(sheet)
        #print(sheetyml)
        image = Image.open(inpath.parent / sheetyml["gfx"])
        palette = image.getpalette()
        if palette is None:
            sys.exit("Provided image does not have a color palette")
        print(len(palette))
        assert(len(palette) % (16*3) == 0)
        pal_count = len(palette) // (16*3)
        ...
        for i in range(pal_count*16):
            f.write(rgb32_to_rgb555(palette[i*3], palette[i*3+1], palette[i*3+2]).to_bytes(2, byteorder='little'))
            size += 2
...
with open(args.outfile + ".seq", "wb") as f:
    for anim in animdef["anims"]:
        ...
        for arrangement, sprites in animyml["arrangements"].items():
            ...
            max_sprite_size = max(max_sprite_size, len(sprites))
            palette_mode = 0
            if args.four:
                if any(x[1] > 1 for x in sheetmeta["parts"]):
                    palette_mode = 1
                elif any(x[1] > 3 for x in sheetmeta["parts"]):
                    palette_mode = 2
            else:
                if any(x[1] > 1 for x in sheetmeta["parts"]):
                    palette_mode = 2
            for sprite in sprites:
                if "palette" in sprite:
                    if args.four:
                        if int(sprite["palette"]) > 1:
                            palette_mode = 1
                        elif int(sprite["palette"]) > 3:
                            palette_mode = 2
                    else:
                        if int(sprite["palette"]) > 1:
                            palette_mode = 2
            ...
            for sprite in sprites:
                ...
                if "palette" in sprite:
                    ...
                    pal = sheetmeta["parts"][sprite["part"]][1]
                
                data |= pal << (11-palette_mode)
                #print(sprite["part"])
        ...
        size += len(frameBytes) + len(arrangementBytes)

print(max_sprite_size)

with open(args.outfile + ".h", "w") as f:
    f.write(f"#ifndef GUARD_{args.outfile}_H\n")
    f.write(f"#define GUARD_{args.outfile}_H\n\n")
    f.write(f"#define ANIM_{args.outfile.upper()}_MAX_SPRITE_COUNT {max_sprite_size}\n\n")
    for k, v in animOffsets.items():
        f.write(f"#define ANIM_{args.outfile.upper()}_{v:X} {v}\n")
        # TODO: add aliases for these defines later
        f.write(f"#define ANIM_{args.outfile.upper()}_{k.upper()} {v}\n\n")
    f.write("#endif")
